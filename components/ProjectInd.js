...
const ProjectInd = ({details}) => {
  ...
  return (
    <div onClick={() => router.push(details.link)} className='text-white flex justify-center items-center text-center grid-web relative  group rounded-[15px] overflow-hidden'>
        ...
        <Image className='w-full hidden lg:block' src={details.lgWeb} alt='Web'/> 
        <Image className='w-full lg:hidden' src={details.web} alt='Web' />
        
        <div className='absolute uppercase'>
            <h2 className='text-[28px] leading-9 mb-7 lg:text-[40px] tracking-[1.4px] lg:tracking-[2px]'>{details.title}</h2>
